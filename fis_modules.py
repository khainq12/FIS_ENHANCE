...
def _mf_med(x: torch.Tensor) -> torch.Tensor:
    ...
    return _clamp01(1.0 - torch.abs(x - 0.5) / 0.3)
...
class FIS_Importance(nn.Module):
    ...
    Rule base: 7 rules (compact but expressive). Output uses zero-order Sugeno
    with fixed consequents for efficiency.
    """
    def __init__(self, eps: float = 1e-8):
        ...
        self.c = torch.tensor([0.90, 0.85, 0.70, 0.50, 0.30, 0.15, 0.65])  # 7 consequents
    ...
    def forward(self, z: torch.Tensor, return_rules: bool = False):
       ...
       # den = rules.sum(dim=1).clamp_min(self.eps) # [B,H,W]---------------------

        den = rules.sum(dim=1)
        den = torch.where(den < 1e-4, torch.ones_like(den), den)
        ...
        # rule_id = torch.argmax(rules, dim=1)       # [B,H,W]
        # return I, rule_id, rules
...
class FIS_PowerAllocation(nn.Module):
    ...
    To keep the rule-base compact and stable, we:
      - Normalize SNR into [0,1] using [snr_min_db, snr_max_db]
      - Use 6 rules depending on (I, SNR) to output A_raw
      - Apply budget interpolation: A = 1 + R*(A_raw - 1) so R=0 => baseline exactly.
    """
    def __init__(
        self,
        a_min: float = 0.70,
        ...
        self.eps = eps
        # 6 consequents for 6 rules
      #  self.c = torch.tensor([1.40, 1.20, 1.10, 1.10, 1.00, 0.90])  # [6] conservative levels-----------
        self.c = torch.tensor([[1.8, 1.5, 1.2, 1.0, 0.8, 0.6]])
    def forward(self, I: torch.Tensor, snr_db: float, budget: float = 1.0, return_rules: bool = False):
        """
        I: [B,H,W] in [0,1]
        snr_db: scalar
        budget: scalar in [0,1]
        returns:
          A: [B,H,W]
          (optional) rule_id_map: [B,H,W]
          (optional) rule_strengths: [B,6,H,W]
        """
        # Normalize SNR -> [0,1]
        s = (float(snr_db) - self.snr_min_db) / (self.snr_max_db - self.snr_min_db + self.eps)
        
        s = torch.tensor(s, device=I.device, dtype=I.dtype).clamp(0.0, 1.0)  # scalar tensor
        # broadcast to [B,H,W]
        ...
        # 6 compact rules:
        # R0: IF I High AND SNR Low  THEN A High
        r0 = _fuzzy_and(iH, sL)
        # R1: IF I High AND SNR Med  THEN A High
        r1 = _fuzzy_and(iH, sM)
        # R2: IF I High AND SNR High THEN A Med
        r2 = _fuzzy_and(iH, sH)
        # R3: IF I Med  AND SNR Low  THEN A MedHigh
        r3 = _fuzzy_and(iM, sL)
        # R4: IF I Med                THEN A Med
        r4 = _fuzzy_and(iM, sM)
        # R5: IF I Low                THEN A Low
        r5 = iL

        rules = torch.stack([r0, r1, r2, r3, r4, r5], dim=1)  # [B,6,H,W]

        c = self.c.to(I.device).view(1, -1, 1, 1)             # [1,6,1,1]
        num = (rules * c).sum(dim=1)
        den = rules.sum(dim=1).clamp_min(self.eps)
        A_raw = num / den

        # SNR-adaptive shrinkage around 1.0:
        # low SNR -> larger delta (stronger allocation), high SNR -> smaller delta (closer to baseline).
        snr_u = float(s)
        # snr_u = torch.sqrt(s)
        delta = self.delta_min + (self.delta_max - self.delta_min) * (1.0 - snr_u)
        # delta = self.delta_min + (self.delta_max - self.delta_min) * snr_u
        A_shrink = 1.0 + delta * (A_raw - 1.0)

        # Budget interpolation: A = 1 + R*(A_shrink - 1) so R=0 => baseline exactly.
        R = float(budget)
        A = 1.0 + R * (A_shrink - 1.0)
        A = A.clamp(min=self.a_min, max=self.a_high)
        # mean-normalize so that average A is 1 (redistribution only)
...
class FIS_SpatialPowerController(nn.Module):
    ...
    modes:
      ...
      - "importance_only": ignore SNR in layer-2 (treat SNR=10dB mid)
      - "snr_only": ignore content importance (I=0.5 constant) -> typically collapses to baseline after PN
      - "linear": linear gating A = clip(1 + alpha*(I-0.5)), then budget interpolation
    """
    def __init__(
        self,
        a_min: float = 0.80,
        ...
        self.pow = FIS_PowerAllocation(
            ...
            snr_min_db=snr_min_db, snr_max_db=snr_max_db, eps=eps
        )
    ...
    def forward(
        ...
        returns:
          A: [B,H,W]
          info dict (optional): {I, A_raw, rule ids, rule strengths}
        """
        info = {}
        mode = str(mode).lower()

        if mode == "snr_only":
            # No content: constant I, will lead to uniform A (and after PN often equals baseline)
        ...
        if mode == "importance_only":
            # Fix SNR at mid (10dB) to remove SNR conditioning
            snr_use = 10.0
        else:
            snr_use = float(snr_db)

        if mode == "linear":
            # Linear gating baseline (content-only) with SNR-damped budget and mean-normalization
            A_raw = 1.0 + self.alpha_linear * (I - 0.5)
            A_raw = A_raw.clamp(min=self.a_min, max=self.a_high)

            # Normalize SNR -> [0,1] for damping (same as power FIS)
            s = (float(snr_use) - self.pow.snr_min_db) / (self.pow.snr_max_db - self.pow.snr_min_db + self.eps)
            s = max(0.0, min(1.0, s))
            R = float(budget)
            R_eff = R * (0.5 + 0.5 * s)
