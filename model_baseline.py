...
class _Encoder(nn.Module):
    def __init__(self, c=1, is_temp=False, P=1, apply_norm: bool = True):
...
class DeepJSCC(nn.Module):
    def __init__(self, c, channel_type='AWGN', snr=None, rician_k: float = 4.0):
        super(DeepJSCC, self).__init__()
        self.encoder = _Encoder(c=c)
        if snr is not None:
            self.channel = Channel(channel_type=channel_type, snr_db=snr, rician_k=rician_k)
    ...
    def change_channel(self, channel_type='AWGN', snr=None, rician_k: float = 4.0):
        if snr is None:
            self.channel = None
        else:
