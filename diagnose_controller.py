...
@torch.no_grad()
def run_one_mode(model: DeepJSCC_FIS, x: torch.Tensor, snr_db: float, budget: float, mode: str) -> Dict[str, Any]:
    z = model.encoder(x)
    A, info = model.controller(z, snr_db=snr_db, budget=budget, mode=mode, return_info=True)
    gain = torch.sqrt(A.clamp_min(model.eps))
...
def main():
    ...
    for mode in parse_modes(args.modes):
        ...
        out = run_one_mode(model, batch, args.snr_db, args.budget, mode)
        raw[mode] = out
