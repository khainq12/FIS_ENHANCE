    ...
    for bidx, (x, _) in enumerate(loader):
        ...
        for mode in modes:
            ...
            if collect_explain:
                ...
                A, _ = fis_model.controller(
                    z, snr_db=snr_db, budget=budget, mode=mode, return_info=True
                )
            else:
                z = fis_model.encoder(x)
                A = fis_model.controller(z, snr_db=snr_db, budget=budget, mode=mode, return_info=False)
