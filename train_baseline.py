...
def main():
    ...
    # 🔥 NEW: enable equalization (GIỐNG FIS)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for x, _ in train_loader:
            ...
            snr = args.snr_min if args.snr_max <= args.snr_min else \
                  random.uniform(args.snr_min, args.snr_max)

            model.change_channel(
                channel_type=args.channel,
                snr=snr,
                rician_k=args.rician_k
            )
        ...
        torch.save(
            model.state_dict(),
            os.path.join(args.save_dir, f"baseline_ep{ep:03d}.pth")
        )

        print(f"Epoch {ep:03d} | loss={running/len(train_loader):.6f} "
              f"| PSNR={psnr_avg:.3f} | time={time.time()-t0:.1f}s")
