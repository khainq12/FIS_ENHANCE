...
def parse_snr_list(values: List[float]) -> List[float]:
    ...
    if len(vals) == 0:
        raise ValueError("eval SNR list must not be empty.")
    return vals
...
def main():
    ...
    ap.add_argument("--rayleigh_equalize", action="store_true", help="Enable Rayleigh equalization (same as FIS)")
    ...
    ap.add_argument("--eval_snr_list", type=float, nargs="+", default=None,
                    help="SNRs used for best-checkpoint selection. Defaults to [snr_min, midpoint, snr_max].")
    ...
    eval_snr_list = parse_snr_list(args.eval_snr_list) if args.eval_snr_list is not None else [
        float(args.snr_min), 0.5 * (float(args.snr_min) + float(args.snr_max)), float(args.snr_max)
    ]
    ...
    for ep in range(1, args.epochs + 1):
        ...
        psnr_avg = evaluate_multi_snr(model, test_loader, device, eval_snr_list, args.channel, args.rician_k)
